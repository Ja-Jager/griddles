 ...
 *
 * @param current_unit_id type:enum adc_unit_t, can select ADC_UNIT_1 OR ADC_UNIT_2.
 * @param current_adc_channel type:enum adc_channel_t, can select from ADC_CHANNEL_0 to ADC_CHANNEL_9.
 * @param demo_offset type:int *, used to Simulate that the board can cook in the occasion of low voltage. 
 * @return Returns ESP_OK on success, or an appropriate error code otherwise.
 */
esp_err_t NTC_thermistor_init(enum adc_unit_t current_unit_id, enum adc_channel_t current_adc_channel, int *demo_offset);
